...
<head>
    ...
    <title>Syllabus Evaluation - CED's Academic Resources Management</title>
    ...
    <style>
        ...
        *{margin:0;padding:0;box-sizing:border-box}
        ...
        .profile-name-nav { color:white; font-size:.75rem; font-weight:500; white-space:nowrap; max-width:120px; overflow:hidden; text-overflow:ellipsis; text-align:center; }
        ...
        .modal-overlay { display:none; position:fixed; inset:0; width:100%; height:100%; background:rgba(0,0,0,.6); backdrop-filter:blur(4px); z-index:2000; align-items:center; justify-content:center; animation:fadeIn .3s ease; }
        .modal-overlay.active { display:flex; }
        .logout-modal, .comment-required-modal { background:white; border-radius:var(--border-radius); box-shadow:0 10px 40px rgba(0,0,0,.3); max-width:450px; width:90%; overflow:hidden; animation:slideIn .3s ease; position:relative; }
        .logout-modal .modal-header, .comment-required-modal .modal-header { background:linear-gradient(135deg, var(--danger-red) 0%, #c82333 100%); color:white; padding:1.5rem 2rem; display:flex; align-items:center; gap:1rem; }
        .comment-required-modal .modal-header { background:linear-gradient(135deg, var(--warning-yellow) 0%, #ffb800 100%); }
        ...
        .modal-btn { padding:12px 24px; border-radius:8px; font-weight:600; font-size:1rem; cursor:pointer; transition:var(--transition); border:none; display:inline-flex; align-items:center; gap:.5rem; }
        ...
        .modal-btn-confirm:hover{ box-shadow:0 4px 12px rgba(220,53,69,.4); }
        .modal-btn-ok{ background:linear-gradient(135deg, var(--accent-blue) 0%, var(--secondary-blue) 100%); color:white; }
        .modal-btn-ok:hover{ box-shadow:0 4px 12px rgba(74,144,226,.4); }
        ...
        /* EVALUATOR REVIEW SECTION - TWO COLUMN LAYOUT */
        .evaluator-review-container{ background:var(--light-gray); padding:20px; border-radius:12px; border-left:4px solid var(--accent-blue); box-shadow:0 2px 8px rgba(0,0,0,.05); }
        ...
        .column-header{ font-size:.9rem; font-weight:600; color:var(--primary-blue); margin-bottom:12px; padding:6px 0; border-bottom:2px solid var(--accent-blue); display:flex; align-items:center; gap:6px; }
        .status-dropdown{ padding:12px 15px; border:2px solid var(--primary-blue); border-radius:8px; font-size:.95rem; transition:var(--transition); outline:none; width:100%; background:white; background-image:url("data:image/svg+xml,%3csvg xmlns='http://www.w3.org/2000/svg' fill='none' viewBox='0 0 20 20'%3e%3cpath stroke='%236b7280' stroke-linecap='round' stroke-linejoin='round' stroke-width='1.5' d='m6 8 4 4 4-4'/%3e%3c/svg%3e"); background-position:right 15px center; background-repeat:no-repeat; background-size:16px; appearance:none; }
        .status-dropdown:focus{ border-color:var(--accent-blue); box-shadow:0 0 0 3px rgba(74,144,226,.1); }
        .comment-textarea{ flex:1; min-height:140px; max-height:200px; resize:vertical; padding:15px; border:2px solid var(--primary-blue); border-radius:8px; font-size:.95rem; transition:var(--transition); outline:none; font-family:inherit; line-height:1.5; background:white; margin-bottom:12px; box-shadow:inset 0 1px 3px rgba(0,0,0,.1); }
        .comment-textarea:focus{ border-color:var(--accent-blue); box-shadow:0 0 0 3px rgba(74,144,226,.15), inset 0 1px 3px rgba(0,0,0,.1); }
        .comment-textarea::placeholder{ color:#999; font-style:italic; }
        .form-buttons{ display:flex; gap:.8rem; flex-shrink:0; }
        .save-btn{ background:linear-gradient(135deg,var(--success-green) 0%,#20c997 100%); color:white; border:none; padding:12px 24px; border-radius:8px; font-size:.95rem; font-weight:600; cursor:pointer; transition:var(--transition); display:flex; align-items:center; gap:.4rem; }
        .save-btn:hover{ transform:translateY(-1px); box-shadow:var(--shadow); }
        .cancel-btn{ background:var(--dark-gray); color:white; border:none; padding:12px 24px; border-radius:8px; font-size:.95rem; font-weight:600; cursor:pointer; transition:var(--transition); display:flex; align-items:center; gap:.4rem; }
        .cancel-btn:hover{ background:#5a6268; transform:translateY(-1px); }

        /* APPROVED DETAILS + COMMENTS */
        .approved-action-cell{ text-align:left; color:var(--dark-gray); padding:1rem; }
        .comment-display-box{ background:white; padding:15px; border-radius:8px; border-left:4px solid var(--accent-blue); box-shadow:0 2px 6px rgba(0,0,0,.08); min-width:300px; max-width:500px; margin-top:10px; }
        .comment-header{ display:flex; align-items:center; gap:8px; color:var(--primary-blue); font-weight:600; font-size:.9rem; margin-bottom:10px; padding-bottom:8px; border-bottom:2px solid var(--light-gray); }
        .comment-header i{ color:var(--accent-blue); font-size:1rem; }
        .comment-text{ font-size:.9rem; line-height:1.6; color:var(--dark-gray); word-wrap:break-word; white-space:pre-wrap; }
        .no-comment-text{ font-style:italic; color:var(--dark-gray); opacity:.6; font-size:.85rem; text-align:center; padding:10px 0; }
        ...
        /* RESPONSIVE */
        @media (max-width:1200px){ .table-container{overflow-x:auto} .data-table{min-width:1400px} }
...
<nav class="navbar navbar-expand-lg">
    <div class="container">
        ...
        <div class="nav-links d-none d-lg-flex">
            ...
            <a href="{{ route('evaluator.syllabus') }}" class="active">
...
<div id="mobileSidebar" class="mobile-sidebar">
    ...
    <a href="{{ route('evaluator.syllabus') }}" class="active" onclick="closeSidebar()">
        <i class="fas fa-book me-2"></i>Syllabus
        @if(isset($pendingSyllabusCount) && $pendingSyllabusCount > 0)
            <span class="nav-badge">{{ $pendingSyllabusCount }}</span>
        @endif
...
<div id="commentRequiredModal" class="modal-overlay" onclick="closeModalOnOverlay(event, 'comment')">
    <div class="comment-required-modal">
        ...
        <div class="modal-body">
            <p>Please add a comment explaining why this syllabus is being returned to the faculty member.</p>
            <div class="modal-actions">
...
<div class="main-container">
    <div class="container">
        ...
        <div class="page-header">
            <h1 class="page-title">Syllabus Evaluation</h1>
            <p class="page-subtitle">Evaluate and provide comments on faculty syllabus submissions</p>
        </div>

        <div class="action-bar">
            <div class="toggle-buttons">
                <button class="toggle-btn" id="pendingBtn" onclick="showPendingSyllabus()">
                    <i class="fas fa-clock"></i>Pending Syllabus
                </button>
                <button class="toggle-btn" id="approvedBtn" onclick="showApprovedSyllabus()">
                    <i class="fas fa-check-circle"></i>Approved Syllabus
                </button>
            ...
            <form class="search-form" action="{{ route('evaluator.syllabus') }}" method="GET">
                @php
                ...
                <input type="text" name="search" class="search-input" placeholder="Search syllabus submissions..." value="{{ request('search') }}" style="width: 320px;">
                ...
                <a href="{{ route('evaluator.syllabus', ['view' => request('view', 'pending')]) }}" class="clear-btn" style="text-decoration:none;"><i class="fas fa-times me-1"></i>Clear</a>
            </form>
        </div>

        <div class="table-container">
            <div class="table-header">
                <h3 id="tableTitle"><i class="fas fa-clock me-2"></i>Pending Syllabus Evaluations</h3>
            </div>
                    ...
                    <tbody id="syllabusTableBody">
                        @forelse ($syllabuses as $syllabus)
                            <tr class="syllabus-row" 
                                ...
                                data-year="{{ $syllabus->academic_year }}"
                                ...
                                <td><span class="badge bg-primary">{{ $syllabus->course_code }}</span></td>
                                <td><strong>{{ $syllabus->course_title }}</strong></td>
                                <td>{{ $syllabus->semester }}</td>
                                <td>{{ $syllabus->academic_year }}</td>
                                <td>{{ $syllabus->campus }}</td>
                                <td>{{ $syllabus->college }}</td>
                                <td><strong><div class="instructor-info">{{ $syllabus->instructor }}</div></strong></td>
                                <td>
                                ...
                                <td class="action-cell">
                                    @if ($syllabus->status !== 'Approved')
                                        <div class="evaluator-review-container">
                                            <form action="{{ route('evaluator.syllabus.update', $syllabus->id) }}" method="POST" class="js-evaluator-review-form">
                                                @csrf
                                                @method('PUT')
                                                ...
                                                <div class="evaluator-review-two-column">
                                                    <div class="evaluator-column-left">
                                                        ...
                                                        <select name="status" class="status-dropdown" required>
                                                            <option value="Pending" {{ $syllabus->status == 'Pending' ? 'selected' : '' }}>Pending Review</option>
                                                            <option value="Approved" {{ $syllabus->status == 'Approved' ? 'selected' : '' }}>Approve Syllabus</option>
                                                            <option value="Returned" {{ $syllabus->status == 'Returned' ? 'selected' : '' }}>Return for Revision</option>
                                                        </select>
                                                    </div>

                                                    <div class="evaluator-column-right">
                                                        <div class="column-header">
                                                            <i class="fas fa-comment"></i>Comment (Will be sent to Faculty)
                                                        </div>
                                                        <textarea name="evaluator_comment" 
                                                                class="comment-textarea" 
                                                                placeholder="Add your detailed evaluation comments here... (optional but recommended for returned syllabus)">{{ $syllabus->evaluator_comment }}</textarea>
                                    ...
                                    @else
                                        <div class="approved-action-cell">
                                            <i class="fas fa-check-circle me-1"></i>Syllabus Approved
                                            <div class="comment-display-box">
                                                <div class="comment-header">
                                                    <i class="fas fa-user-graduate"></i>
                                                    <span>Evaluator's Comment</span>
                                                </div>
                                                @if($syllabus->evaluator_comment)
                                                    <div class="comment-text">{{ $syllabus->evaluator_comment }}</div>
                                                @else
                            ...
                            <tr id="emptyRow">
                                <td colspan="12">
                                    <div class="empty-state">
                                        ...
                                        <h4 id="emptyTitle">No Syllabus Submissions Found</h4>
                                        <p id="emptyMessage">There are no syllabus submissions to evaluate at this time.</p>
                                    </div>
                                </td>
                            </tr>
                        @endforelse
    ...
    <div class="modal-content">
        ...
        <div class="modal-body">
            <div class="mb-3">
                <div class="ratio ratio-16x9">
                    <video id="helpVideo"
                        ...
                        <source src="{{ asset('videos/SyllabusEvaluationTutorial.mp4') }}" type="video/mp4">
                        Your browser does not support HTML5 video.
                        <a href="{{ asset('videos/SyllabusEvaluationTutorial.mp4') }}">Download the video</a>.
                    </video>
                </div>
            </div>
            ...
            <div class="abstract-info">
                <h5>Quick summary</h5>
                <ul class="mb-0">
                    <li><small>Switch between <strong>Pending Syllabus</strong> and <strong>Approved Syllabus</strong>.</small></li>
                    <li><small>Use <strong>Search</strong> to filter by course code, title, semester, year, campus, college, or instructor.</small></li>
...
function applyFiltersAndPagination(){
    ...
    rows.forEach(row=>{
        ...
        if(matchesStatus && matchesSearch && matchesYear){
            matching.push(row);
        }
    });
...
function getEmptyStateCopy(view){
    ...
    if(view==='approved'){
        return {
            title: 'No Approved Syllabus Found',
            message: usingFilters
                ? 'No approved syllabus submissions match your filters.'
                : 'There are no approved syllabus submissions to display at this time.'
        };
    }
    return {
        title: 'No Pending Syllabus Found',
        message: usingFilters
            ? 'No pending syllabus submissions match your filters.'
            : 'There are no pending syllabus submissions to evaluate at this time.'
    };
}
...
/* View toggles (wrappers) */
function showPendingSyllabus(){
    currentView='pending';
    currentPage=1;
    ...
    document.getElementById('tableTitle').innerHTML='<i class="fas fa-clock me-2"></i>Pending Syllabus Evaluations';
    document.getElementById('actionHeader').innerHTML='<i class="fas fa-cog me-1"></i>ACTIONS';
    applyFiltersAndPagination();
}
function showApprovedSyllabus(){
    ...
    document.getElementById('tableTitle').innerHTML='<i class="fas fa-check-circle me-2"></i>Approved Syllabus';
    document.getElementById('actionHeader').innerHTML='<i class="fas fa-info-circle me-1"></i>DETAILS';
    applyFiltersAndPagination();
}
/* Legacy compatibility */
function filterSyllabusByStatus(){ currentPage=1; applyFiltersAndPagination(); }
...
document.addEventListener('DOMContentLoaded',function(){
    ...
    if(view==='approved'){showApprovedSyllabus()} else {showPendingSyllabus()}
